...
```
  ...
  Reducimos el alpha para sacar mejores conclusiones.
  
```{r}
salida_reducida <- multicriterio.metodoELECTRE_I(datos,
                                      pesos.criterios = pesos.criterios,
                                      nivel.concordancia.minimo.alpha = 0.6,
                                     no.se.compensan = c(Inf,Inf,Inf,Inf,Inf, Inf),
                                      que.alternativas = c(1,3,4,5))
salida_reducida
```
...
names(accesibilidad_df) <- c("Accesibilidad", alternativas)
pander(accesibilidad_df)

#PROXIMIDAD
proximidad_df <- data.frame(
  alternativas,
  c(1, 3, 1/5, 2, 1/3),
  c(1/3, 1, 1/7, 1/3, 1/5),
  c(5, 7, 1, 6, 3),
...
names(proximidad_df) <- c("Proximidad", alternativas)
pander(proximidad_df)
